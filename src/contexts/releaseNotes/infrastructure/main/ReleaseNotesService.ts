...
const RELEASE_MANIFEST_OVERRIDE_ENV = 'FREECLI_RELEASE_NOTES_MANIFEST_PATH'
...
function buildReleaseUrl(version: string): string {
  return `https://github.com/Aiden-727/freecli/releases/tag/${encodeURIComponent(
    normalizeVersionTag(version),
  )}`
}

function buildChangelogUrl(): string {
  return 'https://github.com/Aiden-727/freecli/blob/main/CHANGELOG.md'
}
