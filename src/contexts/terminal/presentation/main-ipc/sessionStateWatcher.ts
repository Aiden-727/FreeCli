...
import type { SessionRuntimeMetadata } from '../../../agent/infrastructure/watchers/SessionTurnStateDetector'
...
const SESSION_STATE_WATCHER_RESUME_STANDBY_FALLBACK_MS = 2_500
...
export function createSessionStateWatcherController({
  ...
  const stateWatcherResumeFallbackTimerBySession = new Map<string, NodeJS.Timeout>()
  ...
  const cancelResumeStandbyFallback = (sessionId: string): void => {
    const timer = stateWatcherResumeFallbackTimerBySession.get(sessionId)
    if (!timer) {
      return
    }

    clearTimeout(timer)
    stateWatcherResumeFallbackTimerBySession.delete(sessionId)
  }
  ...
  const broadcastSessionMetadata = (
    sessionId: string,
  ...
  const broadcastRuntimeMetadata = (
    sessionId: string,
    resumeSessionId: string,
    runtimeMetadata: SessionRuntimeMetadata,
  ): void => {
    const previous = stateWatcherLastRuntimeMetadataBySession.get(sessionId)
    if (
      previous?.effectiveModel === runtimeMetadata.effectiveModel &&
      previous?.reasoningEffort === runtimeMetadata.reasoningEffort &&
      previous?.displayModelLabel === runtimeMetadata.displayModelLabel
    ) {
      return
    }

    stateWatcherLastRuntimeMetadataBySession.set(sessionId, runtimeMetadata)
    broadcastSessionMetadata(sessionId, {
      resumeSessionId,
      effectiveModel: runtimeMetadata.effectiveModel,
      reasoningEffort: runtimeMetadata.reasoningEffort,
      displayModelLabel: runtimeMetadata.displayModelLabel,
    })
  }
  ...
  const scheduleResumeStandbyFallback = (
    ...
    if (input.launchMode !== 'resume' || stateWatcherLastBroadcastStateBySession.has(sessionId)) {
      return
    }

    stateWatcherResumeFallbackTimerBySession.set(
      sessionId,
      setTimeout(() => {
        stateWatcherResumeFallbackTimerBySession.delete(sessionId)
        ...
        if (
          ...
          !stateWatcherLastBroadcastStateBySession.has(sessionId)
        ) {
          broadcastSessionState(sessionId, 'standby')
        }
      }, SESSION_STATE_WATCHER_RESUME_STANDBY_FALLBACK_MS),
    )
  }
    ...
    try {
      ...
      if (input.provider === 'opencode') {
        if (!input.opencodeBaseUrl) {
          reportIssue(`[freecli] state watcher missing opencode baseUrl for session ${sessionId}`)
          return
        }
        ...
        const watcher = new OpenCodeSessionStateWatcher({
          ...
          onError: error => {
            ...
            reportIssue(
              `[freecli] state watcher failed for ${input.provider} session ${sessionId}: ${detail}`,
            )
        ...
        watcher.start()
        scheduleResumeStandbyFallback(sessionId, input)
        cancelSessionStateWatcherRetry(sessionId)
        stateWatcherRetryCountBySession.delete(sessionId)
        return
      }
  ...
  const start = (input: SessionStateWatcherStartInput): void => {
    ...
    stateWatcherLastBroadcastStateBySession.delete(input.sessionId)
  ...
  const dispose = (): void => {
    ...
    stateWatcherResumeFallbackTimerBySession.forEach(timer => {
      clearTimeout(timer)
    })
    stateWatcherResumeFallbackTimerBySession.clear()
  }
