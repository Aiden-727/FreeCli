...
import {
  ...
  normalizeTrackHostedAgentPayload,
...
export function registerPtyIpcHandlers(
  ...
  registerHandledIpc<void, TrackHostedTerminalAgentInput>(
    IPC_CHANNELS.ptyTrackHostedAgent,
    async (_event, payload: TrackHostedTerminalAgentInput) => {
      const normalized = normalizeTrackHostedAgentPayload(payload)

      const isApproved = await approvedWorkspaces.isPathApproved(normalized.cwd)
      if (!isApproved) {
        throw createAppError('common.approved_path_required', {
          debugMessage: 'pty:track-hosted-agent cwd is outside approved workspaces',
        })
      }
    ...
    { defaultErrorCode: 'terminal.track_hosted_agent_failed' },
  )
  ...
  return {
    dispose: () => {
      ...
      ipcMain.removeHandler(IPC_CHANNELS.ptyTrackHostedAgent)
