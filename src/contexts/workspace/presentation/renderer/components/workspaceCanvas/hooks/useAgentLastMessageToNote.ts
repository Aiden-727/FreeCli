...
async function readLastAgentMessageWithRetry(
  payload: Parameters<typeof window.freecliApi.agent.readLastMessage>[0],
): Promise<string> {
...
async function readLastAgentMessageWithRetryAttempt(
  ...
  const result = await window.freecliApi.agent.readLastMessage(payload)
...
function resolveLastMessagePayload(
  node: Node<TerminalNodeData>,
): Parameters<typeof window.freecliApi.agent.readLastMessage>[0] | null {
  ...
  if (node.data.kind === 'terminal' && node.data.hostedAgent) {
    const startedAt =
      typeof node.data.hostedAgent.startedAt === 'string'
        ? node.data.hostedAgent.startedAt.trim()
        : typeof node.data.startedAt === 'string'
          ? node.data.startedAt.trim()
          : ''
    ...
    return {
      provider: node.data.hostedAgent.provider,
      cwd: node.data.hostedAgent.cwd,
      startedAt,
      resumeSessionId: node.data.hostedAgent.resumeSessionId ?? null,
    }
  }
...
function isAgentLikeNode(node: Node<TerminalNodeData>): boolean {
  return (node.data.kind === 'agent' && Boolean(node.data.agent)) || Boolean(node.data.hostedAgent)
}
...
export function useWorkspaceCanvasAgentLastMessageCopy({
  ...
  return useCallback(
    async (nodeId: string): Promise<void> => {
      ...
      const payload = resolveLastMessagePayload(node)
      if (!payload) {
        onShowMessage?.(
          t(
            isAgentLikeNode(node)
              ? 'messages.agentLastMessageStartedAtMissing'
              : 'messages.agentLastMessageUnavailable',
          ),
          'warning',
        )
        return
      ...
      try {
        const message = await readLastAgentMessageWithRetry(payload)
        ...
        if (typeof window.freecliApi?.clipboard?.writeText !== 'function') {
          throw new Error(t('common.unknownError'))
        }

        await window.freecliApi.clipboard.writeText(message)
