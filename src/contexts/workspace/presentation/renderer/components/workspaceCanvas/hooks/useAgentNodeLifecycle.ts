  ...
  const launchAgentInNode = useCallback(
    async (nodeId: string, mode: 'new' | 'resume') => {
      ...
      if (launchData.shouldCreateDirectory && launchData.directoryMode === 'custom') {
        await window.freecliApi.workspace.ensureDirectory({ path: launchData.executionDirectory })
      ...
      if (node.data.sessionId.length > 0) {
        ...
        await window.freecliApi.pty.kill({ sessionId: node.data.sessionId })
      ...
      try {
        ...
        if (!isAgentLaunchTokenCurrent(nodeId, launchToken)) {
          void window.freecliApi.pty.kill({ sessionId: launched.sessionId }).catch(() => undefined)
          return
        }
              ...
              data: {
                ...
                title:
                  item.data.titlePinnedByUser === true
                    ? item.data.title
                    : buildAgentNodeTitle(launchData.provider, launched.effectiveModel),
                status:
