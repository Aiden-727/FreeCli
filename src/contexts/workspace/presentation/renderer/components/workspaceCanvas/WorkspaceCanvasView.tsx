...
import { useWorkspaceCanvasLabelColorFilter } from './hooks/useCanvasLabelColorFilter'
import { useWorkspaceCanvasMiddlePan } from './hooks/useCanvasMiddlePan'
...
export function WorkspaceCanvasView({
  ...
  const { labelColorFilter, setLabelColorFilter, usedLabelColors, filteredNodes, filteredEdges } =
    useWorkspaceCanvasLabelColorFilter({
      nodes,
      edges,
      spaces,
    })
  ...
  return (
    <div
      ...
      data-middle-pan-active={isMiddlePanActive ? 'true' : 'false'}
      tabIndex={-1}
      onClick={onCanvasClick}
      onPaste={handleCanvasPaste}
      onDragOver={handleCanvasDragOver}
      onDrop={handleCanvasDrop}
      onMouseDownCapture={handleMiddlePanMouseDownCapture}
      onAuxClick={handleMiddlePanAuxClick}
      onDoubleClickCapture={handleCanvasDoubleClickCapture}
      onPointerDownCapture={event => {
        if (handleMiddlePanPointerDownCapture(event).handled) {
          return
        }
      ...
      onPointerMoveCapture={event => {
        if (handleMiddlePanPointerMoveCapture(event).handled) {
          return
        }
      ...
      onPointerUpCapture={event => {
        if (handleMiddlePanPointerUpCapture(event).handled) {
          return
        }

        handleCanvasPointerUpCapture(event)
      }}
      onPointerCancel={handleMiddlePanPointerCancel}
