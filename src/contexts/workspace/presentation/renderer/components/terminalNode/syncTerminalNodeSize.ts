...
export function syncTerminalNodeSize({
  ...
  lastSyncedContainerSizeRef: MutableRefObject<{ width: number; height: number } | null>
  lastSyncedPtySizeRef: MutableRefObject<{ cols: number; rows: number } | null>
  sessionId: string
  mode?: 'full' | 'foreground'
}): void {
  ...
  const previousContainerSize = lastSyncedContainerSizeRef.current
  const containerSizeChanged =
    !previousContainerSize ||
    previousContainerSize.width !== measuredContainerSize.width ||
    previousContainerSize.height !== measuredContainerSize.height

  if (mode === 'foreground' && !containerSizeChanged) {
  ...
  lastSyncedContainerSizeRef.current = measuredContainerSize
  clampXtermHeightToExactRows(terminal)
  ...
  void window.freecliApi.pty.resize({
    sessionId,
