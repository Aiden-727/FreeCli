...
let scheduledPersistedStateOnResult:
  | ((result: PersistWriteResult, state: PersistedAppState) => void)
  | null = null
...
export function schedulePersistedStateWrite(
  ...
  options: {
    delayMs?: number
    onResult?: (result: PersistWriteResult, state: PersistedAppState) => void
  } = {},
): void {
...
export function flushScheduledPersistedStateWrite(): void {
  ...
  const producedState = producer()

  void writePersistedState(producedState)
    .then(result => {
      onResult?.(result, producedState)
    })
    .finally(() => {
