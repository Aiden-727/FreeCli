...
import type { HostedTerminalAgent } from '@contexts/terminal/domain/hostedAgent'
...
import {
  ...
  deriveTaskAgentSessionRecordBindingId,
...
function ensurePersistedTaskAgentSessionRecords(value: unknown): TaskAgentSessionRecord[] {
  ...
  for (const item of value) {
    ...
    records.push({
      id,
      bindingId:
        normalizeOptionalString(record.bindingId) ?? deriveTaskAgentSessionRecordBindingId(id),
      provider,
      ...resumeBinding,
      prompt,
...
function ensurePersistedAgentData(value: unknown): AgentNodeData | null {
  ...
  return {
    bindingId:
      ...
      deriveAgentNodeBindingId(
        `${provider}:${executionDirectory}:${normalizeOptionalString(record.startedAt) ?? 'persisted'}`,
      ),
    provider,
    prompt,
    model: normalizeOptionalString(record.model),
...
function ensurePersistedHostedAgentData(value: unknown): HostedTerminalAgent | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  ...
  const startedAt = normalizeOptionalString(record.startedAt)
  const model = normalizeOptionalString(record.model)
  const effectiveModel = normalizeOptionalString(record.effectiveModel)
  const reasoningEffort = normalizeOptionalString(record.reasoningEffort)
  const displayModelLabel = normalizeOptionalString(record.displayModelLabel)

  if ((provider !== 'codex' && provider !== 'claude-code') || !cwd) {
    return null
  }

  const fallbackCommand = provider === 'claude-code' ? 'claude' : provider
  const resolvedCommand = command ?? fallbackCommand
  const resolvedStartedAt = startedAt ?? 'persisted'

  const resumeBinding = normalizeResumeSessionBinding(provider, record)
  const stateInput = normalizeOptionalString(record.state)
  const state =
    stateInput === 'active' || stateInput === 'inactive' || stateInput === 'unavailable'
      ? stateInput
      : 'inactive'
  ...
  return {
    ...
    restoreIntent: typeof record.restoreIntent === 'boolean' ? record.restoreIntent : false,
    state,
  }
}
...
function ensurePersistedTaskData(value: unknown): TaskNodeData | null {
  ...
  return {
    ...
    linkedAgentBindingId: normalizeOptionalString(record.linkedAgentBindingId),
