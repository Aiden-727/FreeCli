...
export class SessionTurnStateWatcher {
  ...
  private consumeLine(line: string): void {
    const inspection = inspectSessionLine(this.provider, line)

    if (
      inspection.runtimeMetadata &&
      !areRuntimeMetadataEqual(inspection.runtimeMetadata, this.lastMetadata)
    ) {
    ...
    this.lastState = inspection.state
    this.onState(this.sessionId, inspection.state)
  }
}

function areRuntimeMetadataEqual(
  left: SessionRuntimeMetadata | null,
  right: SessionRuntimeMetadata | null,
): boolean {
  return (
    left?.effectiveModel === right?.effectiveModel &&
    left?.reasoningEffort === right?.reasoningEffort &&
    left?.displayModelLabel === right?.displayModelLabel
  )
}
