...
async function fallbackToFailedAgentTerminal({
  ...
  try {
    const fallback = await window.freecliApi.pty.spawn({
      cwd,
      cols: 80,
      rows: 24,
    })
...
export async function hydrateAgentNode({
  ...
  const persistedResumeSessionId = hasResumeSessionId(node.data.agent.resumeSessionId)
    ? node.data.agent.resumeSessionId.trim()
    : null
  ...
  const sanitizedAgent = resolvedPendingResumeSessionId
    ...
    : persistedResumeSessionId
      ? {
          ...node.data.agent,
          resumeSessionId: persistedResumeSessionId,
          resumeSessionIdVerified: isResumeSessionBindingVerified(node.data.agent),
        }
      : isResumeSessionBindingVerified(node.data.agent)
        ? node.data.agent
        : {
  ...
  const shouldAutoResumeAgent =
    hasActiveAgentStatus && hasResumeSessionId(sanitizedAgent.resumeSessionId)
  const shouldRelaunchBlankAgent =
    hasActiveAgentStatus &&
    !hasResumeSessionId(sanitizedAgent.resumeSessionId) &&
        ...
        data: {
          ...
          title:
            node.data.titlePinnedByUser === true
              ? node.data.title
              : toAgentNodeTitle(sanitizedAgent.provider, restoredAgent.effectiveModel),
          status:
      ...
      return {
        ...
        data: {
          ...
          title:
            node.data.titlePinnedByUser === true
              ? node.data.title
              : toAgentNodeTitle(sanitizedAgent.provider, relaunchedAgent.effectiveModel),
  ...
  if (hasActiveAgentStatus) {
    return fallbackToFailedAgentTerminal({
      node,
      cwd: sanitizedAgent.executionDirectory,
      agent: sanitizedAgent,
      errorMessage: translate('messages.agentRestoreSessionMissing'),
      spawnShell: false,
    })
  }
