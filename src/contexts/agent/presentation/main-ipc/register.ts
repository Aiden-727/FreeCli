    ...
    async (_event, payload: LaunchAgentInput) => {
      ...
      const sessionEnv =
        opencodeServer && normalized.provider === 'opencode'
          ? {
              ...process.env,
              FREECLI_OPENCODE_SERVER_HOSTNAME: opencodeServer.hostname,
              FREECLI_OPENCODE_SERVER_PORT: String(opencodeServer.port),
      ...
      const shouldStartStateWatcher =
        ...
        process.env['FREECLI_TEST_ENABLE_SESSION_STATE_WATCHER'] === '1'
