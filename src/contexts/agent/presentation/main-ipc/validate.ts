...
export function resolveAgentTestStub(
  ...
  const sessionScenario = process.env['FREECLI_TEST_AGENT_SESSION_SCENARIO']?.trim() ?? ''
  const stubScriptPath = process.env['FREECLI_TEST_AGENT_STUB_SCRIPT']?.trim() ?? ''
  ...
  if (process.platform === 'win32') {
    const message = `[freecli-test-agent] ${provider} ${mode ?? 'new'} ${model ?? 'default-model'}`
    return {
