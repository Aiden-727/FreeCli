...
function createMessageMap(): Record<AppErrorCode, string> {
  return {
    ...
    'worktree.api_unavailable':
      'Worktree API is unavailable. Please restart FreeCli and try again.',
    ...
    'terminal.track_hosted_agent_failed': 'Unable to track the hosted terminal agent.',
    ...
    'agent.extensions_get_state_failed': 'Unable to load agent extensions state.',
    'agent.extensions_add_mcp_failed': 'Unable to add the MCP server.',
    'agent.extensions_remove_mcp_failed': 'Unable to remove the MCP server.',
    'agent.extensions_create_skill_failed': 'Unable to create the skill.',
...
export class FreeCliAppError extends Error {
...
export function createAppError(
  ...
  return new FreeCliAppError(descriptor)
}
...
export function formatAppErrorMessage(error: AppErrorDescriptor | FreeCliAppError): string {
  const descriptor = error instanceof FreeCliAppError ? error.toDescriptor() : error
  return APP_ERROR_MESSAGES[descriptor.code] ?? APP_ERROR_MESSAGES['common.unexpected']
}

export function getAppErrorDebugMessage(
  error: AppErrorDescriptor | FreeCliAppError | Error | string | null | undefined,
): string | undefined {
  if (!error) {
    return undefined
  }
...
export function isIpcInvokeResult<T>(value: unknown): value is IpcInvokeResult<T> {
  ...
  const record = value as Record<string, unknown>
  return record.__freecliIpcEnvelope === true && typeof record.ok === 'boolean'
}
