...
function getReleaseNotesApi() {
  return window.freecliApi?.releaseNotes
}
...
export function useWhatsNew({
  ...
  React.useEffect(() => {
    ...
    if (window.freecliApi?.meta?.isTest && !window.freecliApi?.meta?.allowWhatsNewInTests) {
      return
    }
    ...
    if (updateStatus === 'unsupported' && !window.freecliApi?.meta?.allowWhatsNewInTests) {
      return
    }
