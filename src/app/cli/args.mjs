...
export function requireFlagValue(args, flag) {
  ...
  if (!value) {
    exitWithUsage(`[freecli] missing required flag: ${flag} <value>\n`)
  }
...
export function resolveTimeoutMs(argv) {
  ...
  if (!timeoutRaw || timeoutRaw.trim().length === 0 || timeoutRaw.startsWith('-')) {
    exitWithUsage('[freecli] missing required flag: --timeout <ms>\n')
  }

  const timeoutMs = Number(timeoutRaw)
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    exitWithUsage(`[freecli] invalid timeout: ${timeoutRaw}\n`)
  }
