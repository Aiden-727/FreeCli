...
function resolveUserDataCandidates() {
  const candidates = []
  const explicitUserDataDir = process.env.FREECLI_USER_DATA_DIR
  if (explicitUserDataDir && explicitUserDataDir.trim().length > 0) {
    candidates.push(path.resolve(explicitUserDataDir.trim()))
  }

  const appDataDir = resolveAppDataDir()
  candidates.push(path.join(appDataDir, 'freecli-dev'))
  candidates.push(path.join(appDataDir, 'freecli'))
