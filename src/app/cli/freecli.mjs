...
async function main() {
  ...
  if (!connection) {
    process.stderr.write(
      '[freecli] control surface is not running (no valid connection info found).\n',
    )
    process.exit(2)
  }
  ...
  process.stderr.write(`[freecli] unknown command: ${command}\n`)
  printUsage()
  process.exit(2)
}

main().catch(error => {
  process.stderr.write(`[freecli] failed: ${toErrorMessage(error)}\n`)
  process.exit(1)
})
