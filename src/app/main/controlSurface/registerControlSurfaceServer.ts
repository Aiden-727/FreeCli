...
export function registerControlSurfaceServer(deps?: {
  ...
  server.on('error', error => {
    ...
    process.stderr.write(`[freecli] control surface server error: ${detail}\n`)
  })

  server.listen(0, CONTROL_SURFACE_HOSTNAME, () => {
    const address = server.address()
    if (!address || typeof address === 'string') {
      process.stderr.write('[freecli] control surface server did not return a TCP address.\n')
      return
    }
    ...
    pendingConnectionWrite = writeConnectionFile(info).catch(error => {
      const detail = error instanceof Error ? `${error.name}: ${error.message}` : 'unknown error'
      process.stderr.write(`[freecli] failed to write control surface connection file: ${detail}\n`)
    })
  })
