...
export async function runPtyHostUtilityProcessPoc(): Promise<void> {
  if (process.env.NODE_ENV === 'production' && !isTruthyEnv(process.env['FREECLI_PTY_HOST_POC'])) {
    return
  }
  ...
  if (!modulePath) {
    throw new Error(`[freecli] pty-host PoC missing entry: ${modulePathCandidates.join(', ')}`)
  }
  process.stderr.write(`[freecli] pty-host PoC launching: ${modulePath}\n`)

  const child = utilityProcess.fork(modulePath, [], {
    stdio: 'pipe',
    serviceName: 'FreeCli PTY Host (PoC)',
  })
  ...
  child.on('message', rawMessage => {
    ...
    if (message.type === 'exit') {
      process.stderr.write(
        `[freecli] pty-host PoC session exited: ${message.sessionId} code=${message.exitCode}\n`,
      )
      return
    }
  })

  await withTimeout(readyPromise, 5_000, '[freecli] pty-host PoC timed out waiting for ready')
  ...
  const spawnResponse = await withTimeout(
    ...
    10_000,
    '[freecli] pty-host PoC timed out waiting for spawn response',
  )

  if (!spawnResponse.ok) {
    throw new Error(
      `[freecli] pty-host PoC spawn failed: ${spawnResponse.error.name ?? 'Error'}: ${spawnResponse.error.message}`,
    )
  }

  spawnedSessionId = spawnResponse.result.sessionId
  process.stderr.write(`[freecli] pty-host PoC spawned session: ${spawnedSessionId}\n`)
  ...
  await withTimeout(
    ...
    10_000,
    '[freecli] pty-host PoC timed out waiting for output',
  )

  process.stderr.write('[freecli] pty-host PoC observed output OK\n')
  ...
  process.stderr.write('[freecli] pty-host PoC triggering crash\n')
  child.postMessage({ type: 'crash' })

  const exitCode = await withTimeout(
    exitPromise,
    10_000,
    '[freecli] pty-host PoC timed out waiting for host exit',
  )

  process.stderr.write(`[freecli] pty-host PoC host exited (main still alive): code=${exitCode}\n`)
}
