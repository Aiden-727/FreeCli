...
export async function runPtyHostStressTest(): Promise<void> {
  ...
  if (process.platform === 'win32') {
    throw new Error('[freecli] pty-host stress currently supports only macOS/Linux')
  }

  const sessionCount = parsePositiveInt(process.env['FREECLI_PTY_HOST_STRESS_SESSIONS'], 8)
  const bytesPerSession = parsePositiveInt(process.env['FREECLI_PTY_HOST_STRESS_BYTES'], 2_000_000)
  const concurrency = parsePositiveInt(
    process.env['FREECLI_PTY_HOST_STRESS_CONCURRENCY'],
    Math.min(8, sessionCount),
  )
  const timeoutMs = parsePositiveInt(process.env['FREECLI_PTY_HOST_STRESS_TIMEOUT_MS'], 60_000)
  ...
  const ptyHost = new PtyHostSupervisor({
    ...
    createProcess: modulePath => {
      const child = utilityProcess.fork(modulePath, [], {
        stdio: 'pipe',
        serviceName: 'FreeCli PTY Host (Stress)',
      })
      hostPid = child.pid
  ...
  await withTimeout(
    Promise.all(workers),
    timeoutMs,
    `[freecli] pty-host stress timed out while spawning sessions after ${timeoutMs}ms`,
  )
  ...
  const exitPromises = sessionIds.map(sessionId => {
    ...
    return withTimeout(
      exitPromise,
      timeoutMs,
      `[freecli] pty-host stress timed out waiting for session exit after ${timeoutMs}ms: ${sessionId}`,
    )
  })
  ...
  process.stderr.write(
    [
      '[freecli] pty-host stress completed',
