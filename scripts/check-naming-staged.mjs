...
function includesLegacyStoragePrefix(line) {
  // `freecli:*` contains the substring `cove:*`, so we must guard on a word boundary-like separator.
...
function includesCoveTestPrefix(line) {
  // `freecli-test-*` contains the substring `cove-test-*`, so we must guard on a word boundary-like separator.
  return /(^|[^a-zA-Z0-9_])cove-test-/i.test(line)
}

function includesCoveDbFileName(line) {
  // `freecli.db` contains the substring `cove.db`, so we must guard on a word boundary-like separator.
  return /(^|[^a-zA-Z0-9_])cove\.db\b/i.test(line)
}

function includesCoveApiToken(line) {
  // `freecliApi` contains the substring `coveApi`, but without a word boundary before `c`.
...
for (const rawLine of lines) {
  ...
  if (includesCoveApiToken(addedLine)) {
    violations.push({
      file: currentFile,
      rule: 'coveApi',
      hint: 'Use `freecliApi` (global API) instead of `coveApi`.',
  ...
  if (includesCoveLogPrefix(addedLine)) {
    violations.push({
      ...
      hint: 'Use `[freecli]` for log prefixes.',
      line: addedLine.trimEnd(),
    })
  }
  ...
  if (includesCoveTestPrefix(addedLine)) {
    violations.push({
      file: currentFile,
      rule: 'cove-test-*',
      hint: 'Use `freecli-test-*` for test identifiers/prefixes.',
      line: addedLine.trimEnd(),
    })
  }
  ...
  if (includesCoveDbFileName(addedLine)) {
    violations.push({
      file: currentFile,
      rule: 'cove.db',
      hint: 'Use `freecli.db` as the persisted database filename.',
      line: addedLine.trimEnd(),
    })
  }
  ...
  if (includesLegacyStoragePrefix(addedLine)) {
    if (!isExplicitLegacyLine(addedLine)) {
      violations.push({
        file: currentFile,
        rule: 'cove:m0:*',
        hint: 'Use `freecli:m0:*` for new keys. If this is an intentional legacy migration path, add `LEGACY_` or comment `legacy` on the same line.',
        line: addedLine.trimEnd(),
      })
    }
  ...
  if (includesCoveProtocolPrefix(addedLine)) {
    violations.push({
      file: currentFile,
      rule: 'cove:* prefix',
      hint: 'Use `freecli:*` (event/protocol/key prefix) instead of `cove:*`.',
      line: addedLine.trimEnd(),
    })
  }
}
