...
function resolveWindowMode(rawValue) {
  ...
  if (normalized === 'normal') {
    throw new Error(
      '[e2e] FREECLI_E2E_WINDOW_MODE=normal is not allowed because it steals OS focus. Use offscreen/inactive/hidden instead.',
    )
  }
...
async function main() {
  const currentWindowMode = resolveWindowMode(process.env['FREECLI_E2E_WINDOW_MODE'])
  const fallbackWindowMode = resolveFallbackWindowMode(currentWindowMode)

  if (!isTruthyEnv(process.env['FREECLI_E2E_SKIP_BUILD'])) {
  ...
  const firstRun = await runCommand(firstRunArgs, {
    ...process.env,
    FREECLI_E2E_WINDOW_MODE: currentWindowMode,
  })
  if (firstRun.code === 0) {
    process.exit(0)
  }

  if (isTruthyEnv(process.env['FREECLI_E2E_DISABLE_CRASH_FALLBACK'])) {
  ...
  writeStderr(
    `[e2e-fallback] Detected crash-like failure in ${currentWindowMode} mode. Rerunning last failed tests with FREECLI_E2E_WINDOW_MODE=${fallbackWindowMode}...`,
  )

  const fallbackRun = await runCommand(['exec', 'playwright', 'test', '--last-failed'], {
    ...process.env,
    FREECLI_E2E_WINDOW_MODE: fallbackWindowMode,
  })
