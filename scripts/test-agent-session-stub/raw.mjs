...
export async function runRawBracketedPasteEchoScenario() {
  ...
  await new Promise(resolveScenario => {
    ...
    const timeout = setTimeout(() => {
      settle('[freecli-test-paste] timeout')
    }, 8_000)
    ...
    process.stdin.on('data', chunk => {
      ...
      if (typeof bracketedPayload === 'string') {
        settle(`[freecli-test-paste] ${bracketedPayload}`)
        return
      }
  ...
  await new Promise(resolveScenario => {
    ...
    const handleData = chunk => {
      ...
      if (wheelLabel) {
        ...
        settle(`[freecli-test-wheel] ${wheelLabel}${codeSuffix}`)
      }
    }

    const timeout = setTimeout(() => {
      settle('[freecli-test-wheel] timeout')
    }, 8_000)
