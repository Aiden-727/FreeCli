...
export async function runOpenCodeIdleWithMessageScenario(cwd) {
  const hostname = process.env.FREECLI_OPENCODE_SERVER_HOSTNAME?.trim() ?? ''
  const port = Number(process.env.FREECLI_OPENCODE_SERVER_PORT ?? '')
  ...
  const sessionId = `freecli-test-opencode-${Date.now()}`
