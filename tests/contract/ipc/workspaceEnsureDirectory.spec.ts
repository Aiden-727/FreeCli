...
describe('workspace ensureDirectory IPC', () => {
  it('only allows creating directories within approved workspace roots', async () => {
    ...
    const previousTestWorkspace = process.env.FREECLI_TEST_WORKSPACE
    const previousNodeEnv = process.env.NODE_ENV
    process.env.NODE_ENV = 'test'
    process.env.FREECLI_TEST_WORKSPACE = '/tmp/cove-approved-workspace'
    ...
    try {
      ...
      await expect(
        invokeHandledIpc(ensureHandler, null, {
          path: '/tmp/cove-approved-workspace/.freecli/worktrees/demo',
        }),
      ).resolves.toBeUndefined()
      expect(mkdir).toHaveBeenCalledWith(
        '/tmp/cove-approved-workspace/.freecli/worktrees/demo',
      ...
      if (typeof previousTestWorkspace === 'string') {
        process.env.FREECLI_TEST_WORKSPACE = previousTestWorkspace
      } else {
        delete process.env.FREECLI_TEST_WORKSPACE
      }
