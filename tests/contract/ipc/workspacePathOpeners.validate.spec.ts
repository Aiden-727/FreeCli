...
import { getAppErrorDebugMessage, FreeCliAppError } from '../../../src/shared/errors/appError'
...
describe('workspace path opener validation', () => {
  it('accepts the supported opener ids and absolute path formats', async () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(FreeCliAppError)
      expect((error as FreeCliAppError).code).toBe('common.invalid_input')
      expect((error as FreeCliAppError).message).toBe('The request was invalid.')
