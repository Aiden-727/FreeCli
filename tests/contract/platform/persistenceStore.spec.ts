...
describe('PersistenceStore', () => {
  ...
  it(
    ...
    async () => {
      ...
      const dbPath = join(tempDir, 'freecli.db')
      ...
      const backupFiles = files.filter(name => name.startsWith('freecli.db.bak-'))
  ...
  it(
    ...
    async () => {
      ...
      const files = await readdir(tempDir)
      expect(files).toContain('freecli.db.corrupt-2026-02-28T00-00-00-000Z')
      expect(
        await readFile(join(tempDir, 'freecli.db.corrupt-2026-02-28T00-00-00-000Z'), 'utf8'),
      ).toBe('corrupt-db')
    },
    PERSISTENCE_STORE_TEST_TIMEOUT_MS,
  )
  ...
  it(
    ...
    async () => {
      ...
      const store = await createPersistenceStore({
        dbPath: join(tempDir, 'freecli.db'),
