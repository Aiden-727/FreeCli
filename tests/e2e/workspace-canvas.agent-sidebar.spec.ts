...
test.describe('Workspace Canvas - Agent Sidebar', () => {
  test('supports agent controls and sidebar navigation', async () => {
    ...
    try {
      ...
      await expect
        ...
        .toBeLessThan(140)
      await expect(agentNode.locator('.terminal-node__status')).toHaveText(/Standby|待命/)

      const agentHeader = agentNode.locator('.terminal-node__header')
      await expect(agentHeader.locator('[data-testid="terminal-node-model-badge"]')).toHaveText(
        'gpt-5.2-codex',
      )
  ...
  test('shows sidebar agent status and linked task title', async () => {
    ...
    try {
      ...
      await expect(doneItem.locator('.workspace-agent-item__status--agent')).toHaveText(
        /Standby|待命/,
      )
