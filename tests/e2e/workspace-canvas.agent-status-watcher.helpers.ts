...
async function readWorkspaceStateRaw(window: Page): Promise<unknown | null> {
  const raw = await window.evaluate(async () => {
    return await window.freecliApi.persistence.readWorkspaceStateRaw()
  })
...
async function readFirstHarnessAgentSessionId(window: Page): Promise<string | null> {
  return await window.evaluate(() => {
    ...
    return currentWindow.__freecliWorkspaceCanvasTestApi?.getFirstAgentSessionId() ?? null
  })
}
...
async function readHarnessResumeSessionId(
  ...
  return await window.evaluate(sessionId => {
    ...
    return (
      currentWindow.__freecliWorkspaceCanvasTestApi?.getResumeSessionIdByPtySessionId(sessionId) ??
      null
    )
  }, ptySessionId)
}
...
export async function installPtySessionCapture(window: Page): Promise<void> {
  await window.evaluate(() => {
    const captureWindow = window as typeof window & {
      __freecliSeenSessionIds?: string[]
      __freecliPtyCaptureInstalled?: boolean
      __freecliResumeSessionIdByPtySessionId?: Record<string, string | null>
    }

    if (captureWindow.__freecliPtyCaptureInstalled) {
      return
    }

    captureWindow.__freecliSeenSessionIds = []
    captureWindow.__freecliResumeSessionIdByPtySessionId = {}
    const seenSessionIds = captureWindow.__freecliSeenSessionIds
    ...
    window.freecliApi.pty.onData(event => {
...
async function readFirstObservedAgentSessionId(window: Page): Promise<string | null> {
  ...
  return await window.evaluate(() => {
    ...
    const sessionId = captureWindow.__freecliSeenSessionIds?.[0]?.trim() ?? ''
    return sessionId.length > 0 ? sessionId : null
  })
}
...
export async function readObservedResumeSessionId(
  ...
  return await window.evaluate(sessionId => {
    ...
    const resumeSessionId = captureWindow.__freecliResumeSessionIdByPtySessionId?.[sessionId]
...
export async function writeToPty(
  ...
  await window.evaluate(async ({ sessionId, data }) => {
    await window.freecliApi.pty.write({ sessionId, data })
  }, payload)
}
...
export async function writeGeminiSessionFile({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        sessionId,
        projectHash: 'freecli-test-project-hash',
