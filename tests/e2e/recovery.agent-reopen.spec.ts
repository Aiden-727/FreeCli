    ...
    try {
      const { electronApp, window } = await launchApp({
        ...
        env: {
          ...
          FREECLI_TEST_AGENT_SESSION_SCENARIO: 'codex-standby-no-newline',
        },
      })
      ...
      try {
        ...
        await expect(terminalNode).toContainText('[freecli-test-agent] codex resume', {
          timeout: 20_000,
        })
