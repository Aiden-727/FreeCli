    ...
    try {
      const { electronApp, window } = await launchApp({
        ...
        env: {
          ...
          FREECLI_TEST_AGENT_SESSION_SCENARIO: 'gemini-user-then-gemini',
        },
      })
      ...
      try {
        ...
        await expect(terminalNode).toContainText('[freecli-test-agent] gemini resume', {
          timeout: 20_000,
        })
  ...
  test('resumes a task-linked opencode agent after app restart', async () => {
    ...
    try {
      const { electronApp, window } = await launchApp({
        ...
        env: {
          FREECLI_TEST_ENABLE_SESSION_STATE_WATCHER: '1',
          FREECLI_TEST_AGENT_SESSION_SCENARIO: 'opencode-idle-with-message',
        },
      })
      ...
      try {
        ...
        const terminalNode = restartedWindow.locator('.terminal-node').first()
        await expect(terminalNode).toContainText('[freecli-test-agent] opencode resume', {
          timeout: 20_000,
        })
