...
async function createTempRepoWithWorktree(): Promise<{
  ...
  const repoDir = await mkdtemp(path.join(tmpdir(), 'FreeCli Worktree Archive E2E '))

  await runGit(['init'], repoDir)
  await runGit(['config', 'user.email', 'test@example.com'], repoDir)
  await runGit(['config', 'user.name', 'FreeCli Test'], repoDir)
  ...
  const worktreesRoot = path.join(repoDir, '.freecli', 'worktrees with spaces')
  ...
  test('shows a warning when archive cannot remove a locked worktree directory', async () => {
    ...
    try {
      ...
      const { electronApp, window } = await launchApp({
        ...
        env: {
          FREECLI_TEST_WORKSPACE: repoPath,
        },
      })
