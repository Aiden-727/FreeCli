...
const READY_ENV_KEY = 'FREECLI_WINDOWS_COPY_READY_TOKEN'
const SIGINT_ENV_KEY = 'FREECLI_WINDOWS_COPY_SIGINT_TOKEN'
...
test.describe('Workspace Canvas - Terminal Copy (Windows)', () => {
  ...
  test('Ctrl+C copies selected terminal output without sending SIGINT', async () => {
    const readyToken = `FREECLI_WINDOWS_COPY_READY_${Date.now()}`
    const sigintToken = `FREECLI_WINDOWS_COPY_SIGINT_${Date.now()}`
