...
test.describe('Workspace Canvas - Minimap Persistence', () => {
  ...
  test('pans the canvas with the middle mouse button even when dragging on a terminal', async () => {
    const { electronApp, window } = await launchApp()
    ...
    try {
      await clearAndSeedWorkspace(
        ...
        [
          {
            id: 'middle-pan-terminal',
            title: 'terminal-middle-pan',
            position: { x: 120, y: 80 },
            width: 1180,
            height: 760,
          },
        ],
        {
          settings: {
            ...
            focusNodeTargetZoom: 1.6,
          },
        },
      )

      const terminalBody = window
        .locator('.terminal-node', { hasText: 'terminal-middle-pan' })
      ...
      if (!terminalBox) {
        throw new Error('terminal bounding box unavailable for middle pan')
      }

      const startViewport = await readCanvasViewport(window)
      const startPoint = {
        x: terminalBox.x + terminalBox.width * 0.55,
        y: terminalBox.y + terminalBox.height * 0.55,
      }
      const endPoint = {
        x: startPoint.x - 160,
        y: startPoint.y - 110,
      ...
      await window.mouse.move(endPoint.x, endPoint.y, { steps: 18 })
      ...
      await expect
        ...
        .toBeCloseTo(startViewport.zoom, 2)

      const nextViewport = await readCanvasViewport(window)
      expect(Math.abs(nextViewport.x - startViewport.x)).toBeGreaterThan(40)
      expect(Math.abs(nextViewport.y - startViewport.y)).toBeGreaterThan(24)
      expect(nextViewport.zoom).toBeCloseTo(startViewport.zoom, 2)
    } finally {
      await electronApp.close()
    }
  })
