...
import { buildFreeCliContentSecurityPolicy } from '../../../electron.vite.config'
...
describe('buildFreeCliContentSecurityPolicy', () => {
  it('keeps production `style-src` strict while allowing the xterm inline channels it requires', () => {
    const productionPolicy = buildFreeCliContentSecurityPolicy(false)
  ...
  it("keeps 'unsafe-inline' styles in development builds for Vite", () => {
    const developmentPolicy = buildFreeCliContentSecurityPolicy(true)
