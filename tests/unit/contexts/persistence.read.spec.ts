...
describe('workspace persistence (read/normalize)', () => {
        ...
        nodes: [
          {
            ...
            data: {
              ...
              hostedAgent: {
                provider: 'codex',
                launchMode: 'new',
                resumeSessionId: 'session-manual-1',
          ...
          },
          {
            ...
            data: {
              ...
              exitCode: null,
              lastError: null,
              scrollback: '[freecli-test-agent] codex new gpt-5.2-codex\n',
              agent: {
                provider: 'codex',
  ...
  it('excludes ephemeral terminal nodes from durable workspace state', () => {
    const persisted = toPersistedState(
      [
        {
          id: 'workspace-1',
          ...
          spaces: [
            {
              ...
              labelColor: null,
              nodeIds: ['terminal-ephemeral', 'note-1'],
              rect: null,
            },
          ],
    ...
    expect(persisted.workspaces[0]?.nodes.map(node => node.id)).toEqual(['note-1'])
    expect(persisted.workspaces[0]?.spaces[0]?.nodeIds).toEqual(['note-1'])
  })
