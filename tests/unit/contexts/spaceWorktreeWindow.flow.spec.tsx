...
describe('SpaceWorktreeWindow flow', () => {
  ...
  it('opens create and archive views directly without an intermediate home screen', async () => {
    ...
    expect(screen.getByTestId('space-worktree-status')).toHaveTextContent('3 处变更')
    ...
    expect(statusSummary).toHaveBeenCalledWith({
      repoPath: '/repo/.freecli/worktrees/space-1',
    })
  ...
  it('archives a managed worktree and can delete its branch', async () => {
    ...
    await waitFor(() => {
      expect(remove).toHaveBeenCalledWith({
        repoPath: '/repo',
        worktreePath: '/repo/.freecli/worktrees/space-1',
        force: true,
        deleteBranch: true,
      })
  ...
  it('allows mark-mismatch when create is blocked by active windows', async () => {
    ...
    await waitFor(() => {
      expect(onUpdateSpaceDirectory).toHaveBeenCalledWith(
        'space-1',
        '/repo/.freecli/worktrees/space-demo--1a2b3c4d',
  ...
  it('shows archive title summary counts beside the heading', async () => {
    ...
    expect(await screen.findByText('归档 Worktree Space')).toBeVisible()
    expect(screen.getByTestId('space-worktree-archive-summary')).toHaveTextContent(
      '1 个 Agent · 1 个终端 · 1 个任务 · 1 个便签',
    )
  })
    ...
    Object.defineProperty(window, 'freecliApi', {
      ...
      value: {
        worktree: {
          ...
          listWorktrees: vi.fn(async () => ({
            worktrees: [{ path: '/repo/.freecli/worktrees/space-1', head: 'def', branch: 'main' }],
          })),
          ...
          create: vi.fn(async () => ({
            worktree: {
              path: '/repo/.freecli/worktrees/space-demo--1a2b3c4d',
              head: null,
              branch: 'space/demo',
            },
          })),
  ...
  it('archives a managed worktree and surfaces cleanup warnings through app message', async () => {
    ...
    await waitFor(() => {
      ...
      expect(onShowMessage).toHaveBeenCalledWith(
        'Space 已归档，但 worktree 目录未能删除。请关闭仍在使用它的进程，然后手动删除该目录。 Space 已归档，但分支未能删除。',
        'warning',
      )
      expect(onClose).toHaveBeenCalledTimes(1)
    })
