...
describe('ReleaseNotesService', () => {
  afterEach(() => {
    delete process.env.FREECLI_RELEASE_NOTES_MANIFEST_PATH
    vi.resetModules()
    vi.clearAllMocks()
  })

  it('loads localized release notes from the embedded manifest path override', async () => {
    const tempDir = await mkdtemp(resolve(tmpdir(), 'freecli-release-notes-'))
    ...
    process.env.FREECLI_RELEASE_NOTES_MANIFEST_PATH = manifestPath
    vi.doMock('electron', () => ({
  ...
  it('returns a graceful fallback when no manifest is available', async () => {
    ...
    await expect(service.getCurrent({ currentVersion: '9.9.9' })).resolves.toMatchObject({
      provenance: 'fallback',
      items: [],
      compareUrl: 'https://github.com/Aiden-727/freecli/blob/main/CHANGELOG.md',
    })
  })
})
