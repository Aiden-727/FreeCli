...
本文档只定义 FreeCli 的**架构标准与组织方法**，不承担“展示完整目标目录”或“列举具体模块内容”的职责。
...
- 不允许依赖 `React / Electron / CLI / DB / FS / network / window.freecliApi`。
...
FreeCli 将把所有可被外部驱动的能力收敛为一套 **Control Surface**（`command / query / event`），作为 `Desktop(IPC)` / `CLI` / `Web UI` / `Remote Worker` 的共同入口。
...
- 在 renderer 业务流程文件里直接调用 `window.freecliApi`。
- 用持久化结构反向定义业务语义。
