...
从 2026-04-12 起，`quota-monitor` 不再只是一个插件试点，而是 **OpenCove 默认前端视觉基线**。后续所有 renderer 侧新页面、改版页面、插件页、总览卡、配置卡、趋势图、弹窗中的统计区与表单区，默认都必须先对齐这套设计语言，再讨论是否需要偏离。
