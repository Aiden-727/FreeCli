...
- 默认窗口模式：`FREECLI_E2E_WINDOW_MODE=offscreen`
...
```bash
FREECLI_E2E_WINDOW_MODE=inactive|offscreen|hidden pnpm test:e2e
FREECLI_E2E_DISABLE_CRASH_FALLBACK=1 pnpm test:e2e
```
...
- `freecli:m0:workspace-state`（旧版本可能是 `cove:m0:workspace-state`）是否已清理或重建
...
### Windows 下终端偶发闪出控制台窗口

优先检查：

- 是否有后台 CLI 调用遗漏 `windowsHide: true`，尤其是 `spawn / execFile` 的信息探测、标题生成、命名建议这类非交互式命令
- 是否发生在 PTY 关闭/销毁阶段；`node-pty` 的 Windows ConPTY 路径可能额外拉起 helper，表现为黑窗一闪而过
- 若使用 `node-pty`，优先确认是否启用了 Windows-only 的 `conpty.dll` 路径，并保留启动失败后的默认回退，避免因打包差异导致终端完全不可用
- 若 UI 里同时出现“任务像被自动启动”，先区分是任务状态被写坏，还是终端/agent 运行态被误投影到侧边栏或状态栏
