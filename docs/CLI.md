...
本文档定义 FreeCli CLI 的架构约束与可维护规范。

CLI 的定位是：给 Agent/自动化脚本提供一个稳定入口，去驱动 FreeCli 的核心能力，并确保与 Desktop/Web 共享同一套业务真相与不变量。
...
- **显式授权**：使用 token（或等价机制）避免任意本地进程未经授权控制 FreeCli。
