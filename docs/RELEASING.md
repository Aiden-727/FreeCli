...
当前设置页已提供 `清除本地数据` 危险操作，作用范围固定为**当前运行实例**对应的 `userData` 目录：

- 安装版通常是 `%AppData%/freecli`
- 默认开发模式通常是 `%AppData%/freecli-dev`
- 若你显式设置了 `FREECLI_DEV_USER_DATA_DIR`，则以该目录为准

执行后会：

- 删除当前 `userData` 下的本地数据库、插件缓存、日志与恢复数据
- 保留目录本身，避免下次启动因缺失根目录失败
- 自动重启应用，并以全新的本地状态重新初始化

Why：

- Windows 下数据库、日志和插件文件可能被当前进程占用，不能在运行中的同一进程里直接稳定删除
- 当前实现采用“写入重置标记 -> 重启 -> 启动早期清理 `userData`”链路，避免部分删除和文件锁导致的半重置状态

注意：

- 该功能只清当前实例的 `userData`，不会自动同时删除 `freecli` 和 `freecli-dev`
- 如需保留某些本地数据，请先手动备份对应目录
